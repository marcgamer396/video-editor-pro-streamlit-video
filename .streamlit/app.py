...
def process_video(video_path, audio_path, output_path, hdr_settings, fps, use_hdr):
    ...
    if use_hdr:
        ...
        hdr_filter = (
            f"eq=contrast={1 + strength}:brightness={amount * 0.1}:saturation={1 + filters},"
            f"vibrance=intensity={glow},"
            f"unsharp=5:5:{strength * 3}:5:5:{strength * 2},"
            f"eq=gamma={1 + range_val * 0.3}"
        )
...
with st.sidebar:
    ...
    fps = st.selectbox(
        ...
        options=[30, 60],
        index=1,
        help="Select output video frame rate"
    )
    
    st.markdown("---")
    
    st.markdown("### 🔗 Quick Links")
    st.markdown('[🚀 App | StreamLadder](https://app.streamladder.com/editor)', unsafe_allow_html=True)
...
with col1:
    ...
    if audio_option == "Upload Audio File":
        ...
        youtube_url = st.text_input(
            ...
            help="Paste YouTube video URL to extract audio"
        )
...
with col2:
    ...
    video_option = st.radio(
        "Choose video source:",
        ["Upload Video File", "Use Default Video"],
        label_visibility="collapsed"
    )
    ...
    if video_option == "Upload Video File":
        video_file = st.file_uploader(
            ...
            help="Upload your background video"
        )
    else:
        use_default = True
        st.info("📁 Using default video: `D:\\Satisfying 0307 (2).mp4`")
...
if st.button("🎬 Process Video", use_container_width=True):
    ...
    if not audio_file and not youtube_url:
        st.error("⚠️ Please provide an audio source (upload file or YouTube URL)")
    elif not video_file and not use_default:
        st.error("⚠️ Please provide a video source (upload file or use default)")
    else:
        ...
        with st.spinner("🎨 Processing your video..."):
            ...
            try:
                ...
                status_text.text("🎬 Processing video with HDR effects...")
